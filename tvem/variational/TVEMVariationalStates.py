...
def generate_unique_states(n_states: int, H: int, device: to.device = to.device('cpu')) -> Tensor:
    ...
    while len(s_set) < n_states:
        s_set.update({tuple(s) for s in np.random.binomial(1, p=1./H, size=(n_states//2, H))})
    while len(s_set) > n_states:
        s_set.pop()
    return to.from_numpy(np.array(tuple(s for s in s_set), dtype=int)).to(dtype=to.uint8, device=device)

def update_states_for_batch(new_states: Tensor, new_lpj: Tensor, idx: Tensor, all_states: Tensor,
                            all_lpj: Tensor, mstep_factors: Dict[str, Tensor] = None) -> int:
    ...
    TODO Find out why lpj precision decreases for states without substitutions (difference on the order of 1e-15).
    """       
    ...
    sorted_idx = to.flip(to.topk(conc_lpj, k=S, dim=1, largest=True, sorted=True)[1],[1]) # is (batch_size, S)
    ...
    if mstep_factors is not None:
        for key in mstep_factors:
            mstep_factors[key][idx_n,idx_s] = mstep_factors[key][idx_n,flattened_sorted_idx]
...
def set_redundant_lpj_to_low(new_states: Tensor, new_lpj: Tensor, old_states: Tensor):
    """Find redundant states in new_states w.r.t. old_states and set corresponding lpg to low.
    ...
    # old_states must come first for np.unique to discard redundant new_states TODO Check if still holds for to.unique
    old_and_new = to.cat((old_states, new_states), dim=1)
    for n in range(N):            
        uniq_idx = unique_ind(old_and_new[n], dim=0)
        new_uniq_idx = uniq_idx[uniq_idx >= S] - S # indexes of states in new_states[n] that are not in old_states[n]
        mask = to.ones(newS, dtype=to.uint8, device=new_lpj.device)                
        mask[new_uniq_idx.to(device=new_lpj.device)] = 0 # indexes of all non-unique states in new_states (complementary of new_uniq_idx)        
        new_lpj[n][mask] = to.tensor([-1e100],dtype=new_lpj.dtype,device=new_lpj.device) # set lpj of redundant states to an arbitrary low value
...
class TVEMVariationalStates(ABC):
    ...
    def __init__(self, conf: Dict): 
        ...
        self.lpj    = to.empty((conf['my_N'], conf['S']), dtype=conf['dtype_f'], device=conf['device'])
