...
def setup(request):
    class Setup:
        ...
        sigma2_init = to.tensor([2.0], dtype=precision, device=_device)
        ...
        # lpj = \sum_c \log(\pi_c) - 1/(2\sigma^2) (\sum_d (\vec{y}-\vec{W}_c)^2))
        # const = - (D/2) \log(2\pi\sigma^2)
        ...
        true_lpj = to.tensor(
            [
                ...
                [np.log(0.5) - 0.25, np.log(0.5) - 0.25],
            ],
            device=_device,
            dtype=precision,
        )
        true_const = -0.5 * np.log(2 * 2 * math.pi)
        # per datap.
