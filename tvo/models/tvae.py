...
from tvo.utils.model_protocols import Trainable, Sampler, Reconstructor
from tvo.variational.TVOVariationalStates import TVOVariationalStates
from tvo.variational._utils import mean_posterior
from tvo.utils.parallel import all_reduce, broadcast, mpi_average_grads
from tvo.utils import get, CyclicLR
...
def _init_W(
    ...
    if init is None:
        ...
        W = list(
            map(to.nn.init.xavier_normal_, (to.empty(s) for s in W_shapes))
        )  
    else:
        assert (
    ...
    for w in W:
        broadcast(w)
    return [w.to(device=tvo.get_device(), dtype=precision).requires_grad_(True) for w in W]
...
def _init_b(
    ...
    return [b.to(device=tvo.get_device(), dtype=precision).requires_grad_(True) for b in B]
...
def _init_pi(
    ...
    if init is None:
        ...
        pi = init.clone()
    return pi.to(device=tvo.get_device(), dtype=precision).requires_grad_(requires_grad)
...
def _init_sigma2(precision: to.dtype, init: Optional[float], requires_grad: bool) -> to.Tensor:
    ...
    return sigma2.to(device=tvo.get_device(), dtype=precision).requires_grad_(requires_grad)
...
class _TVAE(Trainable, Sampler, Reconstructor):
    ...
    def free_energy(self, idx: to.Tensor, batch: to.Tensor, states: TVOVariationalStates) -> float:
        with to.no_grad():
    ...
    def _optimize_nn_params(
        ...
        objective = mean_posterior(self.log_joint(data, states.K[idx], lpj), lpj_no_grad).sum(
            dim=0
        )  
    ...
    def _accumulate_param_updates(
        self, idx: to.Tensor, data: to.Tensor, states: TVOVariationalStates, mlp_out: to.Tensor
    ) -> None:
        pass
...
class GaussianTVAE(_TVAE):
    def __init__(
        ...
        :param precision: One of to.float32 or to.float64, indicates the floating point precision
                          of model parameters.
        :param min_lr: See docs of tvo.utils.CyclicLR
        :param max_lr: See docs of tvo.utils.CyclicLR
        :param cycliclr_step_size_up: See docs of tvo.utils.CyclicLR
        ...
        ), "Must exclusively specify one one `shape`, (`W_init`, `b_init`), `external_model`"  # either specify shape or (W, b) or external
        ...
        if analytical_sigma_updates:
            self._new_sigma2 = to.zeros(1, dtype=precision, device=tvo.get_device())
        ...
        if analytical_pi_updates:
            self._new_pi = to.zeros(H0, dtype=precision, device=tvo.get_device())
