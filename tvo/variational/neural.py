...
class NeuralVariationalStates(TVOVariationalStates):
        ...
        **kwargs,
        ...
        if self.loss_name == "n_accepted" and self.RBMVAE:
            ...
            self.optimizer = to.optim.Adam(
                list(self.m.variance_stack.parameters()) + list(self.m.alpha_stack.parameters()),
                lr=lr,
            )
    ...
    def neural_update(self, idx: Tensor, batch: Tensor, model: Trainable) -> int:
        ...
        for i in range(self.n_samples):
            ...
            if using_neural_marginal:
                if self.RBMVAE:
                    new_states[:, i, :] = (
                        self.sampling(q[0], q[1]) if hasattr(self, "RBMVAE") else self.sampling(q)
                    )
                else:
                    try:
        ...
        if self._training:
            ...
            if self.loss_name == "LPJ":
                ...
                loss = self.loss_function(
                    states=self.new_states_bfd, pies=q, accepted=vector_subs.float()
                )
    ...
    def tvs_update(self, idx: to.Tensor, batch: to.Tensor, model: Trainable) -> int:
        ...
        new_K_prior = (
            to.rand(batch_size, self.n_samples, H, device=K.device) < model.theta["pies"]
        ).byte()
...
class MLP(to.nn.Module):
    ...
    def build_mlp(self):
        ...
        self.add_module(
            "output_layer",
            to.nn.Linear(
                feature_size_in, self.output_size, dtype=self.precision, device=tvo.get_device()
            ),
        )
