...
def pprint(obj: object = "", end: str = "\n"):
    ...
    """
    if tvo.get_run_policy() == "mpi" and dist.get_rank() != 0:
        return
...
def init_processes(multi_node: bool = False):
    ...
    tvo._set_device(torch.device(device_str))
        ...
        for data in tensors:
            ...
            if comm_rank == 0:
                to_cut_into_chunks = torch.zeros(
                    ((total_length_ceiled,) + other_length),
                    dtype=this_dtype,
                    device=this_device,
                )
                local_start = 0
...
def mpi_average_grads(theta: Dict[str, torch.Tensor]) -> None:
    ...
    :param theta: dictionary with torch.tensors storing TVO model parameters
    """
    if tvo.get_run_policy() != "mpi":
