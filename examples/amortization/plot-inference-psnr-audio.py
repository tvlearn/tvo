...
if __name__ == "__main__":
    ...
    f_infer_mean = "./out/24.11.04-12.00.45-infer/inference.h5"
    f_infer_full = f_infer_mean
    f_evo = "../../../../datasets/veranika/H5files/Bach-00/training.h5"

    evo_psnr = load_vector(f_evo, "snr")[1:]
    evo_F = load_vector(f_evo, "train_F")[1:]

    mean_psnr = load_vector(f_infer_mean, "snr_mean")[1:]
    mean_F = load_vector(f_infer_mean, "test_F")[1:]
    
    full_psnr = load_vector(f_infer_full, "snr_mean")[1:]
    ...
    plt.plot(range(1, len(full_psnr)+1), full_psnr*0+(-6.37), "--", label="data")
    ...
    plt.ylabel("SNR")
    fig.tight_layout()
    plt.savefig(os.path.join(log_path, "bach-PSNR.pdf"))
    plt.close()
    #plt.show()
    ...
    plt.ylabel("ELBO")
    fig.tight_layout()
    plt.savefig(os.path.join(log_path, "bach-ELBO.pdf"))
    plt.close()
    #plt.show()
